...
import {
    Button, Radio, RadioGroup, Chip, Progress,
    Modal, ModalContent, ModalHeader, ModalBody, ModalFooter,
    useDisclosure, Card, CardBody, Image,
} from "@heroui/react";
import {
    attemptResultAtom, selectedAssesmentAnswersAtom,
} from "../../store/test.atom";
import {
    FiArrowLeft, FiArrowRight, FiAlertTriangle,
    FiClock, FiSend, FiMaximize, FiCamera, FiSearch, FiX,
} from "react-icons/fi";
...
export default function AssessmentQuestions() {
    ...
    const submitAttemptMutation = useMutation({
        mutationFn: (id: string) =>
            ApiSDK.AttemptsService.submitAttemptApiV1AttemptsAttemptIdSubmitPost(id),
        onMutate: () => {
    ...
    if (isSubmitting || isTimeUp)
        return (
            <LoadingSequence
                lines={[{ text: "Securing and submitting your responses...", className: "text-2xl font-bold" }]}
            />
        );
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
            ...
            <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
                <div className="max-w-[1440px] mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div
                            className={`flex items-center gap-2 px-4 py-1.5 rounded-xl font-mono font-black text-lg transition-all duration-500 ${timeWarning
                    ...
                    <div className="hidden md:flex items-center gap-2 bg-slate-100 p-1 rounded-2xl">
                        <Chip variant="flat" color={webcamActive ? "success" : "danger"} size="sm" startContent={<FiCamera />}>
                            Webcam
                        </Chip>
                        <Chip variant="flat" color={isFullscreen ? "success" : "danger"} size="sm" startContent={<FiMaximize />}>
                            Fullscreen
                        </Chip>
                        <Chip variant="dot" color={tabSwitchCount >= maxTabSwitches ? "danger" : "warning"} size="sm">
                            Switches: {tabSwitchCount}/{maxTabSwitches}
                        </Chip>
                    </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                        ...
                        <CardBody className="p-0">
                            <div className="flex flex-col">
                                ...
                                {currentQuestion.image_url && (
                                    <div
                                        className="w-full bg-slate-50 border-b border-slate-100 p-6 flex flex-col items-center group relative cursor-zoom-in"
                                    ...
                                    >
                                        ...
                                        />
                                        <p className="mt-3 text-xs font-bold text-slate-400 uppercase tracking-widest group-hover:text-kidemia-secondary transition-colors">
                                ...
                                <div className="p-6 md:p-10">
                                    ...
                                    {/* ── Question text — renderer replaces the h2 ── */}
                                    <div className="text-xl md:text-3xl font-bold text-slate-800 leading-tight mb-8">
                                        <QuestionRenderer
                    ...
                    <div className="hidden lg:flex items-center justify-between gap-4 py-4">
                        ...
                        {!isLastQuestion ? (
                            <Button
                                size="lg"
                                className="bg-kidemia-secondary text-white rounded-2xl font-bold px-12 shadow-lg shadow-blue-200/50"
                                onPress={handleNext}
                    ...
                    {asstQuestions?.require_webcam && (
                        <div className="relative aspect-video rounded-[2.5rem] bg-slate-900 overflow-hidden shadow-2xl border-4 border-white">
                            ...
                            <div className="absolute top-4 left-4">
                                <Chip size="sm" color="danger" className="animate-pulse bg-red-600 text-white font-bold">
                                    LIVE PROCTOR
                                </Chip>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-5 gap-3">
                                {allQuestions.map((_: any, idx: number) => (
                                    <button
                                        ...
                                        className={`h-11 rounded-xl font-bold transition-all ${currentIndex === idx
                                            ? "bg-kidemia-secondary text-white shadow-lg ring-4 ring-blue-50"
                                            : selectedAnswers[idx]
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 p-4 z-40 flex items-center gap-4 shadow-[0_-10px_30px_rgba(0,0,0,0.08)]">
                <Button
                    ...
                    variant="flat"
                    size="lg"
                    className="rounded-2xl min-w-[64px]"
                    onPress={handleBack}
                    isDisabled={currentIndex === 0}
                >
                    <FiArrowLeft size={20} />
                ...
                {!isLastQuestion ? (
                    <Button
                        fullWidth
                        size="lg"
                        className="bg-kidemia-secondary text-white rounded-2xl font-bold"
                        onPress={handleNext}
                        isLoading={saveAnsMutation.isPending}
                        endContent={<FiArrowRight />}
                    >
            ...
            <Modal
                ...
                onOpenChange={onImageZoomChange}
                size="5xl"
                backdrop="blur"
                scrollBehavior="inside"
                closeButton={
                    <Button isIconOnly variant="light" className="m-2 rounded-full">
                        <FiX size={24} />
                    </Button>
                }
            >
                        ...
                        <>
                            ...
                            <ModalBody className="text-center pb-8">
                                ...
                                <p className="text-slate-500">
                                    Violations are logged and may result in automatic disqualification.
                                </p>
                            </ModalBody>
                            <ModalFooter className="justify-center pb-8">
                                <Button
                                    className="px-12 h-14 font-black rounded-2xl bg-kidemia-secondary text-white shadow-xl shadow-blue-100"
                                    onPress={onClose}
                                >
                                    Resume Test
                                </Button>
